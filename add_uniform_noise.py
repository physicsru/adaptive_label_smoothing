...
# python loss_el_cifar.py --lr 0.02 --noise_rate 0.5 --noise_type symmetric --n_epoch 200 --batch-size 128 --dataset cifar10
   ...
   # for batch_idx, (data, target, idx, is_pure, is_corrupt) in enumerate(train_loader):
    ...
    for batch_idx, (data, target,index) in enumerate(train_loader):
        ...
        for param in model.parameters():
            s[i] = -1 + 2 * torch.rand(param.shape)
            param.data = param.data + s[i].cuda()
            i += 1
        loss.backward()
        i = 0
        for param in model.parameters():
...
def main():
    ...
    for epoch in range(1, args.n_epoch + 1):
        l1=train(args, cnn1, device, train_loader, optimizer, epoch, eps=args.eps, nums=num_classes)
        loss.append(l1)
