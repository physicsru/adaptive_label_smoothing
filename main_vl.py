...
TEXT, vocab_size, word_embeddings, train_iter, valid_iter, test_iter = load_data.load_dataset(noise_rate=float(nr))


print(dir(train_iter.train))
...
def train_model(model, train_iter, epoch, eps):
    total_epoch_loss = 0
    total_epoch_acc = 0
...
eee=1-nr
criteria =(-1)* (eee * np.log(eee) + (1-eee) * np.log((1-eee)/(eps-1)))
...
#assert False
np.save("vloss_train_loss_"+str(nr)+" "+str(eps)+".npy",tr_loss)
np.save("vloss_test_acc_"+str(nr)+" "+str(eps)+".npy",te_acc)
