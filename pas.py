...
class PascalSentenceDataSet():
    ...
    def create_correspondence_data(self):
        ...
        for i, img in enumerate(dom('img').items()):
            img_src = "dataset/" + img.attr['src']#.encode()
            txt_src = "sentence/" + img_src[8:-3] + "txt"
            print(type(img_src))
            print(i+1, img_src, txt_src)
            #print("%d => %s" % (i + 1, img_src, txt_src))
            writer.writerow([(i + 1), img_src, txt_src])
